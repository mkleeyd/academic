...
@AllArgsConstructor
public enum ShysCd {
    SHYSCD001("SHYSCD001", "1"),
    SHYSCD002("SHYSCD002", "2"),
    SHYSCD003("SHYSCD003", "3"),
    SHYSCD004("SHYSCD004", "4");
    ...
    public static String findCodeName(String code) {
        String result = Arrays.stream(ShysCd.values())
                .filter(shysCd -> shysCd.code.equals(code))
                .findAny()
                .map(ShysCd::getCodeName)       // .map(mjrCd -> mjrCd.getCodeName())    // 이것도 가능
                .orElse("");
