...
public enum ShtmCd {
    ...
    SHTMCD001("SHTMCD001", "1"),
    SHTMCD002("SHTMCD002", "2");
    ...
    public static String findCodeName(String code) {
        String result = Arrays.stream(ShtmCd.values())
                .filter(shtmCd -> shtmCd.code.equals(code))
                .findAny()
                .map(ShtmCd::getCodeName)       // .map(mjrCd -> mjrCd.getCodeName())    // 이것도 가능
                .orElse("");
